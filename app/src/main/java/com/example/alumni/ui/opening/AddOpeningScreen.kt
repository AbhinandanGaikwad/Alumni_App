...
fun AddOpeningScreen(
    ...
    Column(
        ...
        OutlinedTextField(
            value = appUiState.contactEmail,
            onValueChange = { appViewModel.setContactEmail(it) },
            label = { Text("Email ID for Contact") },
            modifier = modifier
                .fillMaxWidth()
                .padding(bottom = dimensionResource(R.dimen.padding_small))
        )
        ...
        Button(
            ...
            onClick = {
                appViewModel.saveOpeningToFirebase()
            }
        ) {
            Text(
...
fun AddOpeningPreview() {
    AddOpeningScreen(appViewModel = AppViewModel())
}